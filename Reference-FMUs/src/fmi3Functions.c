...
#define MASK_fmi3EnterClockActivationMode (InitializationMode | ClockUpdateMode | ClockActivationMode)
#define MASK_fmi3EnterClockUpdateMode     ClockActivationMode
...
static bool allowedState(ModelInstance *instance, int statesExpected, char *name) {
    ...
    if (!(instance->state & statesExpected)) {
      ...
      logError(instance, "instance state: %d", instance->state) ;
      logError(instance, "expected state: %d", statesExpected) ;
...
/***************************************************
 Functions for Scheduled Execution
 ****************************************************/
fmi3Status fmi3EnterClockActivationMode(fmi3Instance instance) {

    ASSERT_STATE(EnterClockActivationMode);
...
fmi3Status fmi3EnterClockUpdateMode(fmi3Instance instance) {

    ASSERT_STATE(EnterClockUpdateMode);

    S->state = ClockUpdateMode;
